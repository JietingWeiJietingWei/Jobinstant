...
function parseResumeText(raw) {
  ...
  // 2. Job title line: "Title - Subtitle DATE" or "Title DATE"
  //    Pattern: text that ends with a date range on same line → split before date
  //    e.g. "Product Manager – Education Products 2021.10 – 2024.02"
  //    e.g. "IT Advisor - Subject Matter Expert 2017.09 – 2021.06"
...
function renderResume(rawText, changesRaw) {
  ...
  while (i < lines.length) {
    ...
    // e.g. "IBM", "Thomson Reuters", "MetroWall"
    if (trimmed.length <= 40 && /^[A-Z]/.test(trimmed) && !/[@•\-–,|]/.test(trimmed)
        && !/(20\d{2}|19\d{2}|Present)/.test(trimmed)
        && parts.length > 0 && parts[parts.length-1].includes('r-meta')) {
    ...
    if (/[@|•|\||\(]|\d{3}[-.]\d{3}|linkedin|github|http/i.test(trimmed)) {
