...
function saveJDForRewrite() {
  // Save currently visible job's JD to storage so "Tailor Resume" can use it
  const jobTitle = document.querySelector('.job-details-jobs-unified-top-card__job-title, h1')?.textContent?.trim() || '';
  const jdEl = document.querySelector('.jobs-description__content, .jobs-box__html-content, .jobs-description-content__text');
...
setInterval(() => {
  if (location.href !== lastHref) {
    ...
    isRunning = false;
    clearTimeout(autoDebounceTimer);
    clearTimeout(_filterUpdateTimer);
...
let _filterUpdateTimer = null; // debounce filter/UI updates during batch

// ═══════════════════════════════════════════════════════════════
// 🔥 缓存系统 - 记住已评分的职位，提速 50%
// ═══════════════════════════════════════════════════════════════
const scoreCache = new Map();
const CACHE_TTL = 86400000; // 24 hours — job postings rarely change
...
function getFromCache(jobId) {
  const item = scoreCache.get(getCacheKey(jobId));
  if (!item) return null;
  if (Date.now() - item.time > CACHE_TTL) {
    scoreCache.delete(getCacheKey(jobId));
    return null;
  }
  return item.data;
}

function saveToCache(jobId, data) {
  scoreCache.set(getCacheKey(jobId), { data, time: Date.now() });
}
...
const _jdTextCache = new Map(); // jobId → JD text (avoid re-fetching)
const _JD_CACHE_MAX = 150; // cap memory: ~150 × 5KB ≈ 750KB max
function _jdCacheSet(id, text) {
  if (_jdTextCache.size >= _JD_CACHE_MAX) _jdTextCache.delete(_jdTextCache.keys().next().value);
  _jdTextCache.set(id, text);
}

async function getJDFromCard(card) {
  const jobId = getJobIdFromCard(card);

  // Check JD text cache first — avoids re-fetching same JD on retry or revisit
  if (jobId && _jdTextCache.has(jobId)) return _jdTextCache.get(jobId);

  // Method 0 (sync): Read from currently visible panel if this job is already open
  if (jobId) {
    try {
      ...
      if (currentId === jobId) {
        ...
        if (text && text.length > 100) { _jdCacheSet(jobId, text); return text; }
        const allDivs = document.querySelectorAll('div, section');
        for (const el of allDivs) {
          ...
          if (t.length > 300 && t.length < 8000 &&
            ...
            const jd = t.slice(0, 5000);
            _jdCacheSet(jobId, jd);
            return jd;
          }
        }
      }
  ...
  // Race API fetch vs page fetch in parallel — use whichever resolves first
  // Add 5s timeout per fetcher so a hanging request doesn't block everything
  const _withTimeout = (p, ms) => Promise.race([p, new Promise((_, rej) => setTimeout(() => rej('timeout'), ms))]);
  if (jobId) {
    const fetchers = [
      _withTimeout(fetchJDFromAPI(jobId).then(jd => (jd && jd.length > 100) ? jd : Promise.reject('too short')), 5000),
      _withTimeout(fetchJDFromPage(jobId).then(jd => (jd && jd.length > 100) ? jd : Promise.reject('too short')), 5000),
    ];
    try {
      const jd = await Promise.any(fetchers);
      _jdCacheSet(jobId, jd);
      return jd;
    } catch(e) {
      console.warn('[JobInstant] All JD fetch methods failed:', e.errors?.map(e => e.message || e));
    }
  }
...
function observeNewCards() {
  ...
  getJobCards().forEach(card => {
    const id = getJobIdFromCard(card);
    if (!id) return;
    // Re-inject badge for cards that already have cached results (e.g. after page navigation)
    if (batchCache[id] && !card.querySelector('.jdfit-badge-wrap')) {
      injectBadge(card, 'done', batchCache[id]);
    } else if (!batchCache[id]) {
      // Also check scoreCache for results from earlier in the session
      const cached = getFromCache(id);
      if (cached && !card.querySelector('.jdfit-badge-wrap')) {
        batchCache[id] = cached;
        cachedResult[id] = cached;
        injectBadge(card, 'done', cached);
      } else {
        _scrollObserver.observe(card);
      }
    }
  });
  // Update filter UI if any badges were restored
...
async function scoreOneCard(card) {
  ...
  const cached = getFromCache(id);
  if (cached) {
    console.log('[JobInstant] ⚡ 从缓存显示 (0ms)');
    batchCache[id] = cached;
    cachedResult[id] = cached;
    injectBadge(card, 'done', cached);
    applyFilter();
...
// ── Batch abort: allows cancelling an in-progress batch when page changes ──
let _batchAbortController = null;

// ── Main batch scoring ──
async function runBatchScore() {
  // If a batch is already running, abort it first so we can start fresh on new page
  if (batchRunning && _batchAbortController) {
  ...
  const abortSignal = _batchAbortController.signal;
  ensureSpinStyle();
  const btn = document.getElementById('jdfit-score-btn');
  const status = document.getElementById('jdfit-bar-status');
  ...
  const scoreOne = async (card) => {
    // Skip if batch was aborted (page changed) or card was removed from DOM
    if (abortSignal.aborted || !card.isConnected) { done++; updateStatus(); return; }
    const jobId = getJobIdFromCard(card);
    ...
    try {
      ...
      injectBadge(card, 'done', result);
      // Debounce filter updates — avoid 20+ DOM reflows during batch
      clearTimeout(_filterUpdateTimer);
      _filterUpdateTimer = setTimeout(() => { applyFilter(); updateFilterUI(); }, 200);
    } catch(e) {
      if (e.rateLimited) {
  ...
  // Run up to 6 in parallel — no stagger for maximum speed
  const CONCURRENCY = 6;
  const queue = [...cards];
  ...
  // If aborted (page changed), don't try to update old UI elements
  ...
  const strongCount = Object.values(batchCache).filter(r => r.score >= 75).length;
  if (status?.isConnected) status.textContent = `✅ ${done} scored — ${strongCount} strong match${strongCount !== 1 ? 'es' : ''}`;
...
// Track page number separately so page changes don't clear batchCache
let _lastPage = '';
function getPageKey() {
  try {
    return new URLSearchParams(location.search).get('start') || '0';
  } catch(e) { return '0'; }
}
...
function checkAndReinject() {
  ...
  const pageChanged = currentPage !== _lastPage;
  ...
  if (searchChanged) {
    ...
    _lastCardCount = 0;
    batchCache = {};
    if (_batchAbortController) { _batchAbortController.abort(); batchRunning = false; }
    ...
    // Page changed (pagination) — abort batch but keep batchCache for score restoration
    console.log('[JobInstant] Page changed:', currentPage);
    _lastPage = currentPage;
    _lastCardCount = 0;
    if (_batchAbortController) { _batchAbortController.abort(); batchRunning = false; }
    if (_scrollObserver) { _scrollObserver.disconnect(); _scrollObserver = null; }
  ...
  // Auto-observe new cards for scroll-scoring (if batch has been run at least once)
  if (cardCountChanged && Object.keys(batchCache).length > 0) {
